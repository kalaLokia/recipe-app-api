...
def sample_ingrediant(user, name='Cinnamon'):
    """Create and return a sample ingrediant"""
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_create_recipe_with_ingrediants(self):
        """Test creating recipe with ingrediants"""
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload__image_bad_request(self):
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe2 = sample_recipe(user=self.user, title="Beef Chilly")
        recipe3 = sample_recipe(user=self.user, title="Omlate")
        tag1 = sample_tag(user=self.user, name='Curry')
        tag2 = sample_tag(user=self.user, name='Dish')
    ...
    def test_filter_recipes_by_ingrediants(self):
        """Test returning recipes with specific ingrediants"""
        recipe1 = sample_recipe(user=self.user, title="Chicken Chukka")
        recipe2 = sample_recipe(user=self.user, title="Beef Kondattam")
        recipe3 = sample_recipe(user=self.user, title="Mushroom Curry")
        ingrediant1 = sample_ingrediant(user=self.user, name='Chicken')
        ingrediant2 = sample_ingrediant(user=self.user, name='Beef')
        recipe1.ingrediants.add(ingrediant1)
        recipe2.ingrediants.add(ingrediant2)

        res = self.client.get(
            RECIPES_URL,
            {'ingrediants': f'{ingrediant1.id}, {ingrediant2.id}'}
        )
